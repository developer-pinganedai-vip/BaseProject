...
import com.qzsy.baselibrary.widget.app.Constant;
import com.qzsy.baselibrary.widget.bean.MessageEvent;
...
class ScrollNumber extends View {
    ...
    private int measureWidth(int measureSpec) {
        ...
        result = mode == MeasureSpec.AT_MOST ? Math.min(result, val) : result;
        return result + getPaddingLeft() + getPaddingRight() + 4;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCurNum != mTargetNum) {
            ...
            if (mCurNum > 0 && mTargetNum > 0){
                EventBus.getDefault().post(new MessageEvent(Constant.VIEW_SCROLL_FINISH));
            }
        }
